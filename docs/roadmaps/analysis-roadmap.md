...
| Phase 3 | Matrix Event Contract 分析 | 部分完成：04 Matrix integration analysis 已完成；event envelope、task.created、task.accepted、task.rejected、worker.dispatched、proof.submitted、approval.requested、approval.granted、approval.denied、memory.update.proposed baseline 已完成；capability.selected、worker.progress、artifact.submitted、verification.completed、incident.created 缺 |
...
下一步默认推荐仍是 Phase 3：补齐剩余 Matrix event schema / fixture gaps
（capability.selected、worker.progress、artifact.submitted、verification.completed、incident.created）。Capability schema
