...
4. Call `runCodexExecSmoke` with `smoke: true` and an injected process runner.
5. Pass only a minimal explicit environment, such as `PATH`.
6. Capture stdout JSONL, final output, validation log, diff, and proof refs.
7. Validate final output against
   `./schemas/codex/repo-patch-result.schema.json`.
8. Stop before PR creation, push, merge, deploy, Matrix real-service writes, or
   memory writes.
