...
test("workflow maps the CI recovery task graph shape", () => {
  const workflow = ciRecoveryWorkflow();
  ...
  assert.equal(
    workflow.nodes.find((node) => node.kind === "event_ingest").event_ref,
    "event_ci_failure",
  );
  assert.equal(
    workflow.nodes.find((node) => node.id === "node_ci_recovery").capability_ref,
    "capability.ci.recovery@v1",
  );
  assert.equal(
    workflow.nodes.find((node) => node.id === "node_test_run_evidence").capability_ref,
    "capability.test.run@v1",
  );
  assert.equal(
...
test("high-risk recovery PR action requires approval", () => {
  const workflow = ciRecoveryWorkflow();
  const missingApproval = clone(workflow);
  missingApproval.nodes.find((node) => node.id === "node_recovery_pr_create")
    .approval_required = false;

  assert.deepEqual(graphContractErrors(missingApproval), [
    "high-risk external action without approval: node_recovery_pr_create",
  ]);
});
...
test("proof exists before approval and PR creation", () => {
  ...
  assert.deepEqual(graphContractErrors(missingProof), [
    ...
    "missing prior proof requirement: node_recovery_pr_create",
  ]);
});
...
test("workflow uses ci.recovery without repo.patch.codex as recovery capability", () => {
  const workflow = ciRecoveryWorkflow();
  const executionCapabilities = workflow.nodes
    .filter((node) => node.kind === "capability_execution")
    .map((node) => node.capability_ref);

  assert.ok(executionCapabilities.includes("capability.ci.recovery@v1"));
  assert.ok(!executionCapabilities.includes("capability.repo.patch.codex@v1"));
});
